...
select c.nom_categ nom_categ  from categorie c where nb_points =3 ;
...
select t.num_trophee  from trophee t where date_prise between '2052-05-01' and '2052-06-30';
...
select distinct h.nom from habitant h join trophee t on t.num_preneur =h.num_hab 
...
select p.lib_potion ,p.formule, p.constituant_principal   from potion p join fabriquer f on p.num_potion = f.num_potion 
...
select t.date_prise from trophee t 
where t.num_trophee =1;
...
join absorber a on h.num_hab=a.num_hab 
join potion p on a.num_potion =p.num_potion 
where p.lib_potion ='Potion Zen'and quantite >2;

--***
--28. Noms des villages dans lesquels on trouve une resserre (3 lignes)
select v.nom_village,r.num_resserre from village v 
join resserre r on v.num_village =r.num_village 
where r.num_resserre is not null;
...
select h.nom, count(t.num_preneur) from habitant h
